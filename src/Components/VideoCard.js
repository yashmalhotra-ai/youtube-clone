...
import useFormatViewCount from '../utils/useFormatViewCount';

const VideoCard = ({ info }) => {
    let getViewCount = null;
    const { snippet, statistics } = info;
    const { thumbnails, channelTitle, title } = snippet;
    statistics === undefined ? getViewCount = null : getViewCount = statistics?.viewCount;
    const viewCount = useFormatViewCount(getViewCount);



    return (
        <div className='p-2  flex flex-col  text-white cursor-pointer sm:p-6' >
            <div className=' select-none p-2 m-1' >
                <img className='w-full rounded-xl hover:rounded-none transition-all ' src={thumbnails.medium.url} alt="VideoImage" />
            </div>
            <div className=' w-full flex gap-1 '>
                <div className='sm:w-[60px] w-[45px]'>
                ...
                <div className='w-[80%] text-sm'>
                    <h4 className='' title={title}>{title.length===50? title : title.substring(0, 50) +'...' }</h4>
                    <p title={channelTitle} className=' pt-1 opacity-35 font-normal hover:opacity-100'>{channelTitle}</p>
                    {statistics ?
                        <p className='opacity-35 font-normal'>{viewCount} views</p> : null
                    }
